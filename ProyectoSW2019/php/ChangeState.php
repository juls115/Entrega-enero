...
if ($result = $conn->query($sql_query)){
  ...
  if ($invalid){
    ...
    if(!$sql_query = $conn->prepare("UPDATE usuarios SET Estado=? WHERE Correo=?")){
      echo "La petición al servidor no se puede procesar";
    } else {
      foreach($json['usuarios'] as $key => $value){
        if ($requested_states[$key] == 1){
          $estado_nuevo = 0;
        } else{
          $estado_nuevo = 1;
        }
        $sql_query->bind_param("is", $estado_nuevo, $value);
